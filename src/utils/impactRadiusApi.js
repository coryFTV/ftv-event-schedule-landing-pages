...
const AUTH_TOKEN =
  process.env.REACT_APP_IMPACT_RADIUS_AUTH_TOKEN || '9ttadtffjpq#MkVgU2FgPMtzRfj_KRuz';

// Debug logging for environment variables
console.log('API Credentials:');
console.log(
  'ACCOUNT_SID:',
  ACCOUNT_SID ? 'Loaded (length: ' + ACCOUNT_SID.length + ')' : 'Not loaded'
);
console.log(
  'AUTH_TOKEN:',
  AUTH_TOKEN ? 'Loaded (length: ' + AUTH_TOKEN.length + ')' : 'Not loaded'
);
...
const createAuthHeader = () => {
  ...
  const encoded =
    typeof Buffer !== 'undefined' ? Buffer.from(credentials).toString('base64') : btoa(credentials);
