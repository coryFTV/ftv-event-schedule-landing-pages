  ...
  Object.entries(filters).forEach(([key, value]) => {
    if (value) {
      switch (key) {
        case 'Type':
          filteredData = filteredData.filter(
            item => item.adType === value || item.AdType === value
          );
          break;
        case 'MobileReady':
          filteredData = filteredData.filter(
            item => String(item.mobileReady) === value || String(item.MobileReady) === value
          );
          break;
        ...
        case 'SportType':
          filteredData = filteredData.filter(
            item => item.sportType === value || item.SportType === value
          );
          break;
        case 'sport':
          {
            ...
            filteredData = filteredData.filter(
              item =>
                (item.sport && item.sport.toLowerCase() === lowerCaseValue) ||
                (item.league && item.league.toLowerCase() === lowerCaseValue)
            );
          }
          break;
        case 'latino':
          {
            const lowerCaseValue = value.toLowerCase();
            filteredData = filteredData.filter(
              item => item.category && item.category.toLowerCase() === lowerCaseValue
            );
          }
          break;
        default:
          ...
          filteredData = filteredData.filter(item => {
            ...
            return Object.entries(item).some(([itemKey, itemValue]) => {
              if (typeof itemValue === 'string') {
                return (
                  itemKey.toLowerCase() === lowerCaseKey &&
...
export function sortData(data, sortConfig) {
  ...
  return [...data].sort((a, b) => {
    ...
    const aValue =
      a[sortConfig.key] || a[sortConfig.key.charAt(0).toUpperCase() + sortConfig.key.slice(1)];
    const bValue =
      b[sortConfig.key] || b[sortConfig.key.charAt(0).toUpperCase() + sortConfig.key.slice(1)];
