...
function SeriesView({ data, loading: externalLoading, error: externalError, filter = {}, title = "TV Series on Fubo" }) {
    ...
    try {
      ...
      if (filter.type === 'genre' && filter.value) {
        filtered = filtered.filter(series => 
          (series.genre && series.genre.toLowerCase().includes(filter.value.toLowerCase()))
        );
      } else if (filter.type === 'rating' && filter.value) {
        filtered = filtered.filter(series => 
          (series.rating && series.rating === filter.value)
        );
      }
