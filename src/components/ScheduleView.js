...
function ScheduleView({ data, loading, error, filter, title }) {
  ...
  useEffect(() => {
    ...
    if (filter.type === 'regional' && filter.value === true) {
      ...
      filtered = filtered.filter(match => 
        match.country === filter.value
      );
    } else if (filter.type === 'category' && filter.value) {
      // For entertainment vs sports filtering
      if (filter.value === 'entertainment') {
        filtered = filtered.filter(match => 
          !['baseball', 'basketball', 'football', 'soccer', 'hockey', 'cricket', 'tennis', 'golf'].includes(match.sport.toLowerCase())
        );
      }
      ...
      filtered = filtered.filter(match => 
        ...
        (match.league && ['MLB', 'NFL', 'NBA', 'NHL', 'UEFA Champions League'].includes(match.league)) ||
    ...
    if (sortConfig.key) {
      filtered.sort((a, b) => {
        ...
        let aValue = a[sortConfig.key] || a[sortConfig.key === 'starttime' ? 'startTime' : sortConfig.key];
        let bValue = b[sortConfig.key] || b[sortConfig.key === 'starttime' ? 'startTime' : sortConfig.key];
